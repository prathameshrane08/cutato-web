...
useEffect(() => {
  ...
  const todays = useMemo(
    () =>
      all
        ...
        .sort((a, c) => (a.time + a.id).localeCompare(c.time + c.id)),
    [all, today]
  );
  ...
  const pendingCount = useMemo(
    () => all.filter((b) => (b.status ?? "pending") === "pending").length,
    [all]
  );
  ...
  const occupancyToday = useMemo(() => {
    const totalSlots = 24;
    const used = todays.reduce((sum, b) => sum + (b.reservedTimes?.length || 1), 0);
    return Math.min(100, Math.round((used / totalSlots) * 100));
  }, [todays]);

  return (
    <WebShell title="Salon Dashboard" subtitle="Owner portal — overview and quick actions">
      <div className="mx-auto max-w-7xl">
          ...
          <div className="relative flex flex-wrap items-end justify-between gap-6">
            <div>
              ...
              <p className="mt-4 max-w-xl text-white/60">
                Track bookings, revenue, occupancy, staff and daily operations.
              </p>
            </div>
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          <KpiCard icon={<Scissors />} title="Salon" value={salon.salonName || "Cutato"} sub="Current salon profile" />
          <KpiCard icon={<Euro />} title="Revenue today" value={fmtEUR(revenueToday)} sub="Today’s active bookings" />
          <KpiCard icon={<Clock />} title="Occupancy" value={`${occupancyToday}%`} sub="Reserved slot usage" />
          <KpiCard icon={<CalendarDays />} title="Pending" value={`${pendingCount}`} sub="Needs action" />
          <KpiCard icon={<Users />} title="Barbers" value={`${totalBarbers}`} sub="Active staff" />
        </div>
            ...
            <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-neutral-500">
                  Live overview of today’s salon activity.
                </p>
              </div>
          ...
          <aside className="h-fit rounded-[34px] border border-black/10 bg-white p-6 shadow-sm md:p-8 lg:sticky lg:top-28">
            ...
            <div className="mt-6 grid gap-3">
              {upcoming.length === 0 ? (
                ...
                upcoming.map((b) => <MiniBooking key={b.id} booking={b} />)
              )}
            </div>
          </aside>
        </div>
...
function KpiCard({
  ...
  return (
    <div className="rounded-[30px] border border-black/10 bg-white p-6 shadow-sm">
      ...
      <p className="mt-2 truncate text-3xl font-black tracking-[-0.04em]">{value}</p>
      ...
      <div className="flex flex-wrap justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500">
            Customer: {b.userEmail} • Barber: {b.barberName}
          </p>
          <p className="mt-2 text-xs font-bold text-neutral-400">
            Reserved: {b.reservedTimes?.length ? b.reservedTimes.join(", ") : b.time} • Payment:{" "}
...
function MiniBooking({ booking: b }: { booking: Booking }) {
  return (
